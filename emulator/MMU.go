...
func writeByte(GB *GAMEBOY, addr uint16, val uint8) {
	...
	case 0x8000, 0x9000: //Graphics-VRAM
		GB.GPU._vram[addr&0x1FFF] = val

	case 0xA000, 0xB000: //ERAM
		GB.MMU._eram[addr&0x1FFF] = val

	case 0xC000, 0xD000: //WRAM
		GB.MMU._wram[addr&0x1FFF] = val
	...
	case 0xF000: //WRAM-Shadow, I/O, Zero-Page RAM
		...
		case 0xE00: //Graphics-OAM
			if addr < 0xFAE0 {
				GB.GPU._oam[addr&0xFF] = val
			}

		case 0xF00: //Zero-Page
			if addr >= 0xFF80 {
				GB.MMU._zram[addr&0x7F] = val
			}
		}
	}
}
...
func write2Bytes(GB *GAMEBOY, addr uint16, val uint16) {
	valLeft := uint8((val & 0xFF00) >> 8)
	valRight := uint8(val & 0x00FF)
	GB.MMU.writeByte(GB, addr, valLeft)
	GB.MMU.writeByte(GB, addr+1, valRight)
}
